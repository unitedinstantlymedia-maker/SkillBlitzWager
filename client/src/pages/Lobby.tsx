...
export default function Lobby() {
  ...
  const handleFindMatch = () => {
    ...
    dispatch({ type: 'START_MATCH', payload: { id: Math.random().toString(36).substring(7) } });
    setLocation('/match');
  };

  const isBalanceSufficient = state.wallet.balances[state.selectedAsset] >= state.stakeAmount;
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <ToggleGroup type="single" value={state.selectedAsset} onValueChange={handleAssetChange} className="justify-start gap-3">
            {(['USDT', 'ETH', 'TON'] as AssetType[]).map((asset) => (
              <ToggleGroupItem 
                key={asset} 
                value={asset}
                className="h-12 px-6 border border-white/10 data-[state=on]:bg-primary data-[state=on]:text-primary-foreground data-[state=on]:border-primary/50 rounded-lg transition-all"
              >
                {asset}
              </ToggleGroupItem>
            ))}
          ...
          <p className="text-xs font-mono text-muted-foreground ml-1">
            Balance: <span className="text-white">{state.wallet.balances[state.selectedAsset]} {state.selectedAsset}</span>
          </p>
        </div>
        ...
        <Button 
          ...
          disabled={!isBalanceSufficient || state.stakeAmount <= 0}
